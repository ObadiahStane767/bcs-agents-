...
async def plan_next_action(lead_data: Dict[str, Any], state_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if mock_mode:
        return _mock_action_plan(lead_data, state_data)
    
    try:
        return await _openai_action_plan(lead_data, state_data)
    except Exception as e:
        logger.error(f"OpenAI API call failed: {e}")
        return _fallback_action_plan(lead_data, state_data)
...
def get_in_person_followup(lead_data: Dict[str, Any], state_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    # Determine channel and message based on preferred channel
    if preferred in {"whatsapp", "wa"}:
        return {
            "channel": "whatsapp",
...
def _mock_action_plan(lead_data: Dict[str, Any], state_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if is_in_person:
        # Generate in-person followup
        followup = get_in_person_followup(lead_data, state_data)
        
        # Build the action plan for in-person leads
        plan = {
            "action": "send_message",
            ...
            "message": {
                ...
                "whatsapp_text": followup["message"] if followup["channel"] == "whatsapp" else None,
            },
            "metadata": {
                "priority": priority,
                "to_agent": priority >= 8,
                ...
                "thread_key": state_data.get("thread_key") or "",
            },
...
async def _openai_action_plan(lead_data: Dict[str, Any], state_data: Dict[str, Any]) -> Dict[str, Any]:
...
def _fallback_action_plan(lead_data: Dict[str, Any], state_data: Dict[str, Any]) -> Dict[str, Any]:
